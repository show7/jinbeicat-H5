<template>
	<div class="moddelal">
		<header class="u-header"><a class="back" href="javascript:history.back();"><img src="../../../assets/img/left.png"></a>修改交易密码</header>
		<form class="j-tab-forms tn_form" action="/user/deal/domodpass.html" method="post">
            <yd-cell-group>
		        <yd-cell-item>
		            <yd-input slot="right" v-model="input3" required :showSuccessIcon="false" :showErrorIcon="false" regex="passow" placeholder="请输入旧密码" type="password" ref="passow" ></yd-input>
		        </yd-cell-item>
		    </yd-cell-group>
	 	 ...
	 	 methods: {
		 	 ...
		 	 var passworda2=md5(md5(passwd2))
		 	    if(this.ispassword==true&&this.isrepassword==true){
		 	      	...
		 	      	var moddeal = this.serviceAjax('api/member/reset/passwd.do','post',{mobile:mobile,password:passworda2,passwordType:1},token)
	 	 	         //console.log(moddeal); 
