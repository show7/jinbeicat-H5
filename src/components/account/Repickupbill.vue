...
export default {
	 data() {
	  ...
	  mounted: function () {
         ...
         var logs = this.queryAjax('/qapi/user/record/withdraw.do','post',{useTime:0,pageNo:this.nextPage,size:10},token)
         // console.log(logs)
                     ...
                     this.$refs.more.innerText="加载更多";
			            for(var i=0;i<list.length;i++){
			            	...
			            	if(statusa=="SUB"){
			            		...
			            		this.list[listsize +i].status="提现失败"
			            	}else if(statusa=="APL"){
